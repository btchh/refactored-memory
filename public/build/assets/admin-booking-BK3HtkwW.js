class U{constructor(e,t={}){this.container=document.getElementById(e),this.currentDate=new Date,this.onDateSelect=t.onDateSelect||(()=>{}),this.selectedDate=null,this.bookingCounts={},this.countsUrl=t.countsUrl||null}async render(){if(!this.container)return;const e=this.currentDate.getFullYear(),t=this.currentDate.getMonth();this.countsUrl&&await this.loadBookingCounts(e,t+1);const s=document.getElementById("current-month");s&&(s.textContent=new Date(e,t).toLocaleDateString("en-US",{month:"long",year:"numeric"}));const o=new Date(e,t,1).getDay(),a=new Date(e,t+1,0).getDate(),y=new Date;y.setHours(0,0,0,0);const m=document.getElementById("calendar-grid");if(m){m.innerHTML="",["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].forEach(d=>{const f=document.createElement("div");f.className="text-center font-bold text-sm text-gray-500 py-2",f.textContent=d,m.appendChild(f)});for(let d=0;d<o;d++)m.appendChild(document.createElement("div"));for(let d=1;d<=a;d++){const f=new Date(e,t,d);f.setHours(0,0,0,0);const k=`${e}-${String(t+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`,w=f<y,x=this.bookingCounts[k]||0,p=document.createElement("div");p.className=`calendar-day font-semibold relative ${w?"disabled":"available"}`,x>0?p.innerHTML=`
        ...
        `).join(""),s.classList.remove("hidden"))}displayEmptyState(){const e=document.getElementById("user-bookings-list"),t=document.getElementById("user-bookings-section");e&&(e.innerHTML='<p class="text-gray-500 text-center py-4">No bookings found</p>'),t&&t.classList.add("hidden")}getStatusBadgeClass(e){return{pending:"badge-warning",in_progress:"badge-info",completed:"badge-success",cancelled:"badge-error"}[e]||"badge-neutral"}async cancelBooking(e){if(!confirm("Are you sure you want to cancel this booking?"))return;const t=prompt("Cancellation reason (optional):");try{(await B.delete(`/admin/bookings/${e}`,{reason:t})).success&&(alert("Booking cancelled successfully"),this.loadUserBookings(this.userId),this.onBookingUpdate())}catch(s){console.error("Error cancelling booking:",s),alert("Failed to cancel booking")}}async changeStatus(e,t){if(t)try{(await B.patch(`/admin/bookings/${e}/status`,{status:t})).success&&(alert("Status updated successfully"),this.loadUserBookings(this.userId),this.onBookingUpdate())}catch(s){console.error("Error updating status:",s),alert("Failed to update status")}}openRescheduleModal(e){const t=document.getElementById("reschedule-modal"),s=document.getElementById("reschedule-booking-id");s&&(s.value=e),t&&t.showModal&&t.showModal()}async submitReschedule(e,t,s){try{if((await B.post(`/admin/bookings/${e}/reschedule`,{booking_date:t,booking_time:s})).success){alert("Booking rescheduled successfully");const a=document.getElementById("reschedule-modal");a&&a.close&&a.close(),this.loadUserBookings(this.userId),this.onBookingUpdate()}}catch(o){console.error("Error rescheduling:",o),alert("Failed to reschedule booking")}}setupRescheduleForm(){const e=document.getElementById("reschedule-form");e&&e.addEventListener("submit",async t=>{t.preventDefault();const s=document.getElementById("reschedule-booking-id")?.value,o=document.getElementById("reschedule-date")?.value,a=document.getElementById("reschedule-time")?.value;s&&o&&a&&await this.submitReschedule(s,o,a)})}init(){this.setupRescheduleForm(),window.bookingManager=this}}class P{constructor(e={}){this.selectElement=document.getElementById(e.selectId||"booking_time"),this.slotsUrl=e.slotsUrl||"/api/calendar/slots"}async loadSlots(e){if(this.selectElement){this.selectElement.innerHTML='<option value="">Loading slots...</option>',this.selectElement.disabled=!0;try{const t=await B.get(`${this.slotsUrl}?date=${e}`);if(this.selectElement.innerHTML='<option value="">Select time slot</option>',t.slots&&t.slots.length>0){const s=t.slots.filter(o=>o.available);s.length===0?this.selectElement.innerHTML='<option value="">No slots available</option>':s.forEach(o=>{const a=document.createElement("option");a.value=o.time,a.textContent=o.formatted||o.time,this.selectElement.appendChild(a)})}else this.selectElement.innerHTML='<option value="">No slots available</option>'}catch(t){console.error("Error loading time slots:",t),this.selectElement.innerHTML='<option value="">Error loading slots. Please try again.</option>'}finally{this.selectElement.disabled=!1}}}clear(){this.selectElement&&(this.selectElement.innerHTML='<option value="">Select time slot</option>')}}document.addEventListener("DOMContentLoaded",()=>{const h=window.bookingData;if(!h){console.error("Booking data not found");return}const{services:e,products:t,routes:s,csrf:o}=h,a=new P({selectId:"booking_time",slotsUrl:s.slots}),y=new F({bookingsUrl:s.bookings,userBookingsUrl:s.userBookings,csrf:o,onBookingUpdate:()=>{}});y.init();const m=new U("calendar-container",{countsUrl:s.bookingCounts,onDateSelect:async n=>{const i=document.getElementById("booking_date");i&&(i.value=n),await d(n),a.loadSlots(n)}});m.init();async function d(n){const i=document.getElementById("date-bookings-section"),l=document.getElementById("date-bookings-list"),r=document.getElementById("selected-date-display"),c=document.getElementById("booking-form"),u=document.getElementById("empty-state"),v=document.getElementById("bookings-count-badge");l&&(l.innerHTML=`
            ...
            `);try{const b=await(await fetch(`${s.bookingsByDate}?date=${n}`)).json();if(!b.success||!i||!l)return;if(u&&(u.style.opacity="0",setTimeout(()=>u.classList.add("hidden"),300)),r){const g=new Date(n+"T00:00:00");r.textContent=g.toLocaleDateString("en-US",{weekday:"long",year:"numeric",month:"long",day:"numeric"})}v&&(v.textContent=b.bookings.length),i.classList.remove("hidden"),setTimeout(()=>{i.style.opacity="1"},10),c&&c.classList.add("hidden"),b.bookings.length===0?l.innerHTML=`
                ...
                `:l.innerHTML=b.bookings.map((g,D)=>`
                ...
                `).join("")}catch(E){console.error("Failed to load date bookings:",E)}}function f(n){return{pending:"bg-yellow-100 text-yellow-700",in_progress:"bg-blue-100 text-blue-700",completed:"bg-green-100 text-green-700",cancelled:"bg-red-100 text-red-700"}[n]||"bg-gray-100 text-gray-700"}function k(){const n=document.getElementById("booking-form"),i=document.getElementById("date-bookings-section");n&&(n.classList.remove("hidden"),setTimeout(()=>{n.scrollIntoView({behavior:"smooth",block:"start"})},100)),i&&(i.style.opacity="0",setTimeout(()=>{i.classList.add("hidden")},300))}window.showBookingForm=k;const w=document.getElementById("show-add-form-btn");w&&w.addEventListener("click",k),window.viewBooking=async n=>{try{const l=await(await fetch(s.bookingDetails.replace("__ID__",n))).json();if(l.success){const r=l.booking,c=document.getElementById("view-booking-content");c.innerHTML=`
                ...
                `,document.getElementById("view-booking-modal").showModal()}else alert("Failed to load booking details")}catch(i){console.error("Error:",i),alert("Failed to load booking details")}},window.rescheduleBooking=n=>{y.openRescheduleModal(n)},window.editWeightFromView=(n,i)=>{document.getElementById("view-booking-modal").close(),document.getElementById("edit-weight-booking-id").value=n,document.getElementById("edit-booking-weight").value=i.replace(" kg","")||"",document.getElementById("edit-weight-modal").showModal()};const x=document.getElementById("edit-weight-form");x&&x.addEventListener("submit",async n=>{n.preventDefault();const i=document.getElementById("edit-weight-booking-id").value,l=document.getElementById("edit-booking-weight").value;try{const c=await(await fetch(s.updateWeight.replace("__ID__",i),{method:"POST",headers:{"Content-Type":"application/json","X-CSRF-TOKEN":bookingData.csrf},body:JSON.stringify({weight:l})})).json();c.success?(alert(c.message),document.getElementById("edit-weight-modal").close(),selectedDate&&loadBookingsByDate(selectedDate)):alert(c.message||"Failed to update weight")}catch(r){console.error("Error:",r),alert("Failed to update weight")}}),window.cancelBooking=async n=>{if(confirm("Are you sure you want to cancel this booking?"))try{const l=await(await fetch(`${s.bookings}/${n}`,{method:"DELETE",headers:{"X-CSRF-TOKEN":o,"Content-Type":"application/json"}})).json();if(l.success){alert("Booking cancelled successfully");const r=document.getElementById("booking_date");r&&r.value&&await d(r.value),m.render()}else alert("Failed to cancel booking: "+l.message)}catch(i){console.error("Failed to cancel booking:",i),alert("Failed to cancel booking")}};const p=new j({servicesData:e,productsData:t});p.init(),new _({inputId:"user-search",resultsId:"user-search-results",searchUrl:s.userSearch,onUserSelect:n=>{const i=document.getElementById("user_id"),l=document.getElementById("pickup_address"),r=document.getElementById("latitude"),c=document.getElementById("longitude");i&&(i.value=n.id),l&&(l.value=n.address),r&&(r.value=n.latitude||""),c&&(c.value=n.longitude||"");const u=document.getElementById("selected-user-info");u&&(document.getElementById("selected-user-name").textContent=n.name,document.getElementById("selected-user-email").textContent=n.email,document.getElementById("selected-user-phone").textContent=n.phone,document.getElementById("selected-user-address").textContent=n.address,u.classList.remove("hidden")),y.loadUserBookings(n.id);const v=document.getElementById("bookings-user-name");v&&(v.textContent=n.name)}}).init();const C=document.getElementById("clear-user");C&&C.addEventListener("click",()=>{const n=document.getElementById("user_id"),i=document.getElementById("selected-user-info"),l=document.getElementById("user-bookings-section");n&&(n.value=""),i&&i.classList.add("hidden"),l&&l.classList.add("hidden")});const M=document.querySelectorAll('input[name="service_type"]'),T=document.getElementById("pickup-address-section"),I=document.getElementById("pickup_address");function $(){const n=document.querySelector('input[name="service_type"]:checked')?.value;T&&(n==="dropoff"?(T.classList.add("hidden"),I&&I.removeAttribute("required")):(T.classList.remove("hidden"),I&&I.setAttribute("required","required")))}M.forEach(n=>{n.addEventListener("change",$)}),$();const L=document.getElementById("clear-form");L&&L.addEventListener("click",()=>{const n=document.getElementById("booking-form");n&&n.reset(),p.reset();const i=document.getElementById("booking_date"),l=i?i.value:null;i&&(i.value=""),a.clear();const r=document.getElementById("booking-form"),c=document.getElementById("date-bookings-section"),u=document.getElementById("empty-state"),v=document.getElementById("user-bookings-section");r&&r.classList.add("hidden"),l&&c?(c.classList.remove("hidden"),c.style.opacity="1"):(c&&c.classList.add("hidden"),u&&(u.classList.remove("hidden"),u.style.opacity="1")),v&&v.classList.add("hidden");const E=document.getElementById("user_id"),b=document.getElementById("selected-user-info");E&&(E.value=""),b&&b.classList.add("hidden"),l||m.reset()})});let S=!1;window.handleBookingSubmit=function(h){if(S)return h.preventDefault(),!1;S=!0;const e=h.target.querySelector('button[type="submit"]');return e&&(e.disabled=!0,e.innerHTML='<svg class="animate-spin h-5 w-5 mr-2 inline" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> Submitting...'),setTimeout(()=>{S=!1,e&&(e.disabled=!1,e.innerHTML="Submit Booking")},3e3),!0};
