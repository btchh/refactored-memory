class A{constructor(e,t={}){this.container=document.getElementById(e),this.currentDate=new Date,this.onDateSelect=t.onDateSelect||(()=>{}),this.selectedDate=null,this.bookingCounts={},this.countsUrl=t.countsUrl||null,this.allowPastDates=t.allowPastDates!==!1}async render(){if(!this.container)return;const e=this.currentDate.getFullYear(),t=this.currentDate.getMonth();this.countsUrl&&await this.loadBookingCounts(e,t+1);const s=document.getElementById("current-month");s&&(s.textContent=new Date(e,t).toLocaleDateString("en-US",{month:"long",year:"numeric"}));const o=new Date(e,t,1).getDay(),a=new Date(e,t+1,0).getDate(),u=new Date;u.setHours(0,0,0,0);const p=document.getElementById("calendar-grid");if(p){p.innerHTML="",["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].forEach(d=>{const f=document.createElement("div");f.className="text-center font-bold text-sm text-gray-500 py-2",f.textContent=d,p.appendChild(f)});for(let d=0;d<o;d++)p.appendChild(document.createElement("div"));for(let d=1;d<=a;d++){const f=new Date(e,t,d);f.setHours(0,0,0,0);const k=`${e}-${String(t+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`,b=f<u,L=f.getTime()===u.getTime(),x=this.bookingCounts[k]||0,y=document.createElement("div");let w="calendar-day font-semibold relative cursor-pointer ";if(L&&(w+="today ring-2 ring-wash ring-offset-1 "),b?w+="past bg-gray-100 text-gray-600 hover:bg-gray-200 ":w+="available ",y.className=w,x>0){const $=b?"bg-gray-500":"bg-primary-600";y.innerHTML=`
                    <span>${d}</span>
                    <span class="absolute top-1 right-1 ${$} text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-bold">${x}</span>
                `}else y.textContent=d;y.dataset.date=k,y.dataset.day=d,y.dataset.count=x,y.dataset.isPast=b,y.onclick=()=>this.selectDate(k,y,d,b),p.appendChild(y)}}}async loadBookingCounts(e,t){if(this.countsUrl)try{const o=await(await fetch(`${this.countsUrl}?year=${e}&month=${t}`)).json();o.success&&(this.bookingCounts=o.counts)}catch(s){console.error("Failed to load booking counts:",s)}}selectDate(e,t,s,o=!1){this.selectedDate=e,this.selectedIsPast=o;const a=document.getElementById("calendar-grid");a&&a.querySelectorAll(".calendar-day").forEach(u=>{u.classList.remove("selected")}),t.classList.add("selected"),this.onDateSelect(e,o)}navigateMonth(e){const t=new Date(this.currentDate);t.setMonth(t.getMonth()+e),this.currentDate=t,this.render()}setupNavigation(){const e=document.getElementById("prev-month"),t=document.getElementById("next-month");e&&(e.addEventListener("click",()=>{this.navigateMonth(-1)}),e.disabled=!1,e.classList.remove("btn-disabled","opacity-50","cursor-not-allowed")),t&&t.addEventListener("click",()=>{this.navigateMonth(1)})}reset(){this.selectedDate=null,this.selectedIsPast=!1;const e=document.getElementById("calendar-grid");e&&e.querySelectorAll(".calendar-day").forEach(t=>{t.classList.remove("selected")})}init(){this.render(),this.setupNavigation()}}class N{constructor(e={}){this.servicesData=e.servicesData||{},this.productsData=e.productsData||{},this.selectedServices=[],this.selectedProducts=[],this.onTotalChange=e.onTotalChange||(()=>{})}loadServices(e){const t=document.getElementById("services-container");if(!t)return;const s=this.servicesData[e]||[];if(s.length===0){t.innerHTML='<p class="text-gray-500 col-span-2">No services available</p>';return}t.innerHTML=s.map(o=>`
        ...
        `).join(""),s.classList.remove("hidden"))}displayEmptyState(){const e=document.getElementById("user-bookings-list"),t=document.getElementById("user-bookings-section");e&&(e.innerHTML='<p class="text-gray-500 text-center py-4">No bookings found</p>'),t&&t.classList.add("hidden")}getStatusBadgeClass(e){return{pending:"badge-warning",in_progress:"badge-info",completed:"badge-success",cancelled:"badge-error"}[e]||"badge-neutral"}cancelBooking(e){const t=document.querySelector('meta[name="csrf-token"]')?.content||"";typeof window.showCancelModal=="function"?window.showCancelModal(e,{type:"admin",cancelUrl:"/admin/bookings/__ID__/cancel",csrfToken:t,onSuccess:()=>{this.loadUserBookings(this.userId),this.onBookingUpdate()}}):(console.error("showCancelModal function not available"),window.Toast?.error("Cancel function not loaded. Please refresh the page."))}async changeStatus(e,t){if(t)try{(await S.patch(`/admin/bookings/${e}/status`,{status:t})).success&&(window.Toast?.success("Status updated successfully"),this.loadUserBookings(this.userId),this.onBookingUpdate())}catch(s){console.error("Error updating status:",s),window.Toast?.error("Failed to update status")}}openRescheduleModal(e){const t=document.getElementById("reschedule-modal"),s=document.getElementById("reschedule-booking-id"),o=document.getElementById("reschedule-date"),a=document.getElementById("reschedule-time");s&&(s.value=e),o&&(o.value=""),a&&(a.innerHTML='<option value="">Select a date first</option>'),t&&t.showModal&&t.showModal()}async submitReschedule(e,t,s){try{if((await S.post(`/admin/bookings/${e}/reschedule`,{booking_date:t,booking_time:s})).success){window.Toast?.success("Booking rescheduled successfully");const a=document.getElementById("reschedule-modal");a&&a.close&&a.close(),this.loadUserBookings(this.userId),this.onBookingUpdate()}}catch(o){console.error("Error rescheduling:",o),window.Toast?.error("Failed to reschedule booking")}}setupRescheduleForm(){const e=document.getElementById("reschedule-form");if(!e)return;const t=document.getElementById("reschedule-date");t&&t.addEventListener("change",async s=>{await this.loadTimeSlots(s.target.value)}),e.addEventListener("submit",async s=>{s.preventDefault();const o=document.getElementById("reschedule-booking-id")?.value,a=document.getElementById("reschedule-date")?.value,u=document.getElementById("reschedule-time")?.value;o&&a&&u&&await this.submitReschedule(o,a,u)})}async loadTimeSlots(e){const t=document.getElementById("reschedule-time");if(t){if(!e){t.innerHTML='<option value="">Select a date first</option>';return}t.innerHTML='<option value="">Loading...</option>';try{const s=window.bookingData?.routes?.slots||"/admin/api/calendar/slots",o=await S.get(`${s}?date=${e}`);o.success&&o.slots&&o.slots.length>0?t.innerHTML='<option value="">Select time slot</option>'+o.slots.map(a=>`<option value="${a.time}">${a.display}</option>`).join(""):t.innerHTML='<option value="">No available slots</option>'}catch(s){console.error("Error loading time slots:",s),t.innerHTML='<option value="">Error loading slots</option>'}}}init(){this.setupRescheduleForm(),window.bookingManager=this}}class V{constructor(e={}){this.selectElement=document.getElementById(e.selectId||"booking_time"),this.slotsUrl=e.slotsUrl||"/api/calendar/slots"}async loadSlots(e){if(this.selectElement){this.selectElement.innerHTML='<option value="">Loading slots...</option>',this.selectElement.disabled=!0;try{const t=await S.get(`${this.slotsUrl}?date=${e}`);if(this.selectElement.innerHTML='<option value="">Select time slot</option>',t.slots&&t.slots.length>0){const s=t.slots.filter(o=>o.available);s.length===0?this.selectElement.innerHTML='<option value="">No slots available</option>':s.forEach(o=>{const a=document.createElement("option");a.value=o.time,a.textContent=o.formatted||o.time,this.selectElement.appendChild(a)})}else this.selectElement.innerHTML='<option value="">No slots available</option>'}catch(t){console.error("Error loading time slots:",t),this.selectElement.innerHTML='<option value="">Error loading slots. Please try again.</option>'}finally{this.selectElement.disabled=!1}}}clear(){this.selectElement&&(this.selectElement.innerHTML='<option value="">Select time slot</option>')}}document.addEventListener("DOMContentLoaded",()=>{const h=window.bookingData;if(!h){console.error("Booking data not found");return}const{services:e,products:t,routes:s,csrf:o}=h,a=new V({selectId:"booking_time",slotsUrl:s.slots}),u=new O({bookingsUrl:s.bookings,userBookingsUrl:s.userBookings,csrf:o,onBookingUpdate:()=>{}});u.init();const p=new A("calendar-container",{countsUrl:s.bookingCounts,allowPastDates:!0,onDateSelect:async(n,r)=>{const l=document.getElementById("booking_date");if(l&&(l.value=n),await d(n,r),!r)a.loadSlots(n);else{const i=document.getElementById("booking_time");i&&(i.innerHTML='<option value="">Past date - view only</option>',i.disabled=!0)}}});p.init();async function d(n,r=!1){const l=document.getElementById("date-bookings-section"),i=document.getElementById("date-bookings-list"),c=document.getElementById("selected-date-display"),m=document.getElementById("booking-form"),v=document.getElementById("empty-state"),I=document.getElementById("bookings-count-badge");i&&(i.innerHTML=`
                <div class="flex items-center justify-center py-8">
                    <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-primary-600"></div>
                    <span class="ml-3 text-gray-600">Loading bookings...</span>
                </div>
            `);try{const C=await(await fetch(`${s.bookingsByDate}?date=${n}`)).json();if(!C.success||!l||!i)return;if(v&&(v.style.opacity="0",setTimeout(()=>v.classList.add("hidden"),300)),c){const g=new Date(n+"T00:00:00");c.textContent=g.toLocaleDateString("en-US",{weekday:"long",year:"numeric",month:"long",day:"numeric"})}I&&(I.textContent=C.bookings.length),l.classList.remove("hidden"),setTimeout(()=>{l.style.opacity="1"},10),m&&m.classList.add("hidden"),C.bookings.length===0?i.innerHTML=`
                    <div class="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                        <svg class="w-16 h-16 text-gray-400 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
                        </svg>
                        <h4 class="text-lg font-semibold text-gray-900 mb-2">No Bookings ${r?"Found":"Yet"}</h4>
                        <p class="text-gray-600 mb-4">${r?"No bookings were made for this date":"This date is available for new bookings"}</p>
                        ${r?"":`
                ...
                `:i.innerHTML=C.bookings.map((g,F)=>`
                    <div class="border border-gray-200 rounded-lg p-4 hover:shadow-lg hover:border-primary-300 transition-all duration-200 bg-white animate-slide-in" style="animation-delay: ${F*50}ms">
                ...
                `).join("")}catch(T){console.error("Failed to load date bookings:",T)}}function f(n){return{pending:"bg-yellow-100 text-yellow-700",in_progress:"bg-blue-100 text-blue-700",out_for_delivery:"bg-purple-100 text-purple-700",completed:"bg-green-100 text-green-700",cancelled:"bg-red-100 text-red-700"}[n]||"bg-gray-100 text-gray-700"}function k(){const n=document.getElementById("booking-form"),r=document.getElementById("date-bookings-section");n&&(n.classList.remove("hidden"),setTimeout(()=>{n.scrollIntoView({behavior:"smooth",block:"start"})},100)),r&&(r.style.opacity="0",setTimeout(()=>{r.classList.add("hidden")},300))}window.showBookingForm=k;const b=document.getElementById("show-add-form-btn");b&&b.addEventListener("click",k),window.viewBooking=async n=>{try{const l=await(await fetch(s.bookingDetails.replace("__ID__",n))).json();if(l.success){const i=l.booking,c=document.getElementById("view-booking-content");c.innerHTML=`
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="col-span-2">
                            ...
                            <ul class="space-y-1">
                                ${i.services.map(m=>`<li class="text-sm text-gray-900">• ${m.name} - ${m.price}</li>`).join("")}
                            </ul>
                        </div>
                        `:""}
                        ${i.products.length>0?`
                        <div class="col-span-2">
                            ...
                            <ul class="space-y-1">
                                ${i.products.map(m=>`<li class="text-sm text-gray-900">• ${m.name} - ${m.price}</li>`).join("")}
                            </ul>
                        </div>
                        `:""}
                        <div class="col-span-2">
                ...
                `,document.getElementById("view-booking-modal").showModal()}else window.Toast?.error("Failed to load booking details")}catch(r){console.error("Error:",r),window.Toast?.error("Failed to load booking details")}},window.rescheduleBooking=n=>{u.openRescheduleModal(n)},window.editWeightFromView=(n,r)=>{document.getElementById("view-booking-modal").close(),document.getElementById("edit-weight-booking-id").value=n,document.getElementById("edit-booking-weight").value=r.replace(" kg","")||"",document.getElementById("edit-weight-modal").showModal()};const L=document.getElementById("edit-weight-form");L&&L.addEventListener("submit",async n=>{n.preventDefault();const r=document.getElementById("edit-weight-booking-id").value,l=document.getElementById("edit-booking-weight").value;try{const c=await(await fetch(s.updateWeight.replace("__ID__",r),{method:"POST",headers:{"Content-Type":"application/json","X-CSRF-TOKEN":bookingData.csrf},body:JSON.stringify({weight:l})})).json();c.success?(window.Toast?.success(c.message),document.getElementById("edit-weight-modal").close(),selectedDate&&loadBookingsByDate(selectedDate)):window.Toast?.error(c.message||"Failed to update weight")}catch(i){console.error("Error:",i),window.Toast?.error("Failed to update weight")}}),window.cancelBooking=n=>{typeof window.showCancelModal=="function"?window.showCancelModal(n,{type:"admin",cancelUrl:`${s.bookings}/__ID__/cancel`,csrfToken:o,onSuccess:async()=>{const r=document.getElementById("booking_date");r&&r.value&&await d(r.value),p.render()}}):(console.error("showCancelModal function not available"),window.Toast?.error("Cancel function not loaded. Please refresh the page."))};const x=new N({servicesData:e,productsData:t});x.init(),new R({inputId:"user-search",resultsId:"user-search-results",searchUrl:s.userSearch,onUserSelect:n=>{const r=document.getElementById("user_id"),l=document.getElementById("pickup_address"),i=document.getElementById("latitude"),c=document.getElementById("longitude");r&&(r.value=n.id),l&&(l.value=n.address),i&&(i.value=n.latitude||""),c&&(c.value=n.longitude||"");const m=document.getElementById("selected-user-info");m&&(document.getElementById("selected-user-name").textContent=n.name,document.getElementById("selected-user-email").textContent=n.email,document.getElementById("selected-user-phone").textContent=n.phone,document.getElementById("selected-user-address").textContent=n.address,m.classList.remove("hidden")),u.loadUserBookings(n.id);const v=document.getElementById("bookings-user-name");v&&(v.textContent=n.name)}}).init();const w=document.getElementById("clear-user");w&&w.addEventListener("click",()=>{const n=document.getElementById("user_id"),r=document.getElementById("selected-user-info"),l=document.getElementById("user-bookings-section");n&&(n.value=""),r&&r.classList.add("hidden"),l&&l.classList.add("hidden")});const $=document.querySelectorAll('input[name="service_type"]'),B=document.getElementById("pickup-address-section"),E=document.getElementById("pickup_address");function M(){const n=document.querySelector('input[name="service_type"]:checked')?.value;B&&(n==="dropoff"?(B.classList.add("hidden"),E&&E.removeAttribute("required")):(B.classList.remove("hidden"),E&&E.setAttribute("required","required")))}$.forEach(n=>{n.addEventListener("change",M)}),M();const P=document.querySelectorAll('input[name="booking_type"]'),D=document.getElementById("customer-selection-section"),_=document.getElementById("user_id");function j(){const n=document.querySelector('input[name="booking_type"]:checked')?.value;if(D&&_)if(n==="walkin"){D.classList.remove("hidden"),_.removeAttribute("required"),B&&(B.classList.add("hidden"),E&&E.removeAttribute("required"));const r=document.querySelector('input[name="service_type"][value="dropoff"]');r&&(r.checked=!0,r.closest(".bg-white").classList.add("hidden"))}else{D.classList.remove("hidden"),_.setAttribute("required","required");const r=document.querySelector('input[name="service_type"]')?.closest(".bg-white");r&&r.classList.remove("hidden"),M()}}P.forEach(n=>{n.addEventListener("change",j)}),j();const H=document.getElementById("clear-form");H&&H.addEventListener("click",()=>{const n=document.getElementById("booking-form");n&&n.reset(),x.reset();const r=document.getElementById("booking_date"),l=r?r.value:null;r&&(r.value=""),a.clear();const i=document.getElementById("booking-form"),c=document.getElementById("date-bookings-section"),m=document.getElementById("empty-state"),v=document.getElementById("user-bookings-section");i&&i.classList.add("hidden"),l&&c?(c.classList.remove("hidden"),c.style.opacity="1"):(c&&c.classList.add("hidden"),m&&(m.classList.remove("hidden"),m.style.opacity="1")),v&&v.classList.add("hidden");const I=document.getElementById("user_id"),T=document.getElementById("selected-user-info");I&&(I.value=""),T&&T.classList.add("hidden"),l||p.reset()})});let U=!1;window.handleBookingSubmit=function(h){if(U)return h.preventDefault(),!1;U=!0;const e=h.target.querySelector('button[type="submit"]');return e&&(e.disabled=!0,e.innerHTML='<svg class="animate-spin h-5 w-5 mr-2 inline" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> Submitting...'),setTimeout(()=>{U=!1,e&&(e.disabled=!1,e.innerHTML="Submit Booking")},3e3),!0};
