<x-layout>
    <x-slot:title>Chat with {{ $admin->fname }}</x-slot:title>
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="card p-4 mb-4">
            <div class="flex items-center">
                ...
                <div class="w-10 h-10 bg-primary-100 text-primary-600 rounded-full flex items-center justify-center mr-3 font-medium">
                    {{ strtoupper(substr($admin->fname, 0, 1)) }}{{ strtoupper(substr($admin->lname, 0, 1)) }}
                </div>
                <div>
                    <h2 class="font-semibold text-gray-900">{{ $admin->fname }} {{ $admin->lname }}</h2>
    ...
    <script>
        ...
        const sendUrl = '{{ route('user.messages.send', $admin->id) }}';
        const fetchUrl = '{{ route('user.api.messages', $admin->id) }}';
