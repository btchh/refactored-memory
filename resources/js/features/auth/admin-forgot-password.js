...
class AdminForgotPassword extends MultiStepForm {
    constructor() {
        super({
            totalSteps: 3,
            completedStepClass: 'w-10 h-10 mx-auto rounded-full bg-blue-600 text-white flex items-center justify-center font-bold',
    ...
    async handleSendOtp() {
        ...
        try {
            const result = await this._sendOTPRequest(
                document.querySelector('[data-send-otp-url]')?.dataset.sendOtpUrl || '/admin/send-password-reset',
                { phone },
                button,
                'phone-error'
            );
    ...
    async handleVerifyOtp() {
        ...
        try {
            const result = await this._verifyOTPRequest(
                document.querySelector('[data-verify-otp-url]')?.dataset.verifyOtpUrl || '/admin/verify-password-reset',
                { 
    ...
    async handlePasswordReset() {
        ...
        try {
            const response = await fetch(
                document.querySelector('[data-reset-password-url]')?.dataset.resetPasswordUrl || '/admin/reset-password',
                {
                    method: 'POST',
                    headers: {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.getElementById('send-otp-form') && 
        ...
        window.adminForgotPassword = new AdminForgotPassword();
    }
});
